...
import { CvExamplesGallery } from "./CvExamplesGallery";
...
export default async function CvExamplesPage() {
  const supabase = createClient();
  const { data: auth } = await supabase.auth.getUser();
  const userEmail = auth.user?.email ?? null;
  const tier = (auth.user?.user_metadata as { tier?: string } | undefined)?.tier ?? null;
  const examplesDir = path.join(process.cwd(), "..", "templates", "CVs");
  let examples: CvExample[] = [];
  ...
  try {
    ...
    examples = files
      ...
      .map((file) => ({
        file,
        name: toTitle(file),
        src: `/api/cv-examples/${encodeURIComponent(file)}`,
      }));
  } catch {
    examples = [];
  }
      ...
      <main className="mx-auto w-full max-w-7xl px-6 py-16 lg:px-8">
        ...
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div className="max-w-2xl">
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-white/60">CV Examples</p>
            <h1 className="mt-3 text-4xl font-semibold leading-tight text-white lg:text-5xl">
              Review real-world CV examples by role.
            </h1>
            <p className="mt-4 text-base leading-relaxed text-white/72">
              Open any example to see a complete CV structure and phrasing tailored to specific roles.
            </p>
          </div>
        ...
        {examples.length === 0 ? (
          <section className="mt-10">
            <div className="rounded-2xl border border-white/10 bg-white/5 p-8">
              <p className="text-lg font-semibold text-white">No CV examples found.</p>
              <p className="mt-2 text-sm text-white/65">Add example PDFs to load this gallery.</p>
            </div>
          </section>
        ) : (
          <CvExamplesGallery examples={examples} />
        )}
      </main>
